...
export const useSEO = (options: UseSEOOptions = {}) => {
  ...
  useEffect(() => {
    ...
    const seoData: SEOData & UseSEOOptions = {
      ...
      description:
        options.dynamicDescription ||
        options.description ||
        routeSEO.description,
      keywords:
        options.dynamicKeywords || options.keywords || routeSEO.keywords,
    };
    ...
    updateMetaTag(
      "og:image",
      `${baseUrl}${seoData.image || "/estrella-de-la-vida.png"}`,
      true
    );
  ...
  return {
    updateTitle: (newTitle: string) => {
      document.title = newTitle.includes("|")
        ? newTitle
        : `${newTitle} | Alumnos R1 `;
    },
