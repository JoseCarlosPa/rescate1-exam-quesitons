...
export default function RandomQuestion({ onClose }: { onClose?: () => void }) {
    ...
    return (
        <div className={`relative px-6 py-5 bg-white rounded-xl shadow-xl border border-orange-200 transition-all duration-500 ${animateIn ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            ...
            {feedback && (
                <div className={`mb-4 p-2 rounded-lg text-center font-medium ${isCorrect ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                    {feedback}
                </div>
            )}
