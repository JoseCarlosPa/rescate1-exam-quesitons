                    ...
                    {filteredCases.map((simulatorCase) => {
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex-1">
                                            ...
                                            <span
                                                className={`inline-block px-3 py-1 text-xs font-medium rounded-full border ${getDifficultyColor(simulatorCase.difficulty)}`}>
                        {getDifficultyLabel(simulatorCase.difficulty)}
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg mb-4">
                                        <h4 className="text-sm font-semibold text-gray-900 mb-2">Información del
                                            Paciente</h4>
                                        <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                                            <div>
                                                <span
                                                    className="font-medium">Edad:</span> {simulatorCase.patientInfo.age} años
                                            </div>
                                            <div>
                                                <span
                                                    className="font-medium">Sexo:</span> {simulatorCase.patientInfo.gender}
                                            </div>
                                        </div>
                                        <div className="mt-2 text-sm text-gray-600">
                                            <span
                                                className="font-medium">Motivo:</span> {simulatorCase.patientInfo.chiefComplaint}
                                        </div>
                                    </div>

                                    {/* Objetivos de aprendizaje */}
                                    <div className="mb-4">
                                        <h4 className="text-sm font-semibold text-gray-900 mb-2">Objetivos de
                                            Aprendizaje</h4>
                                        <ul className="text-sm text-gray-600 space-y-1">
                                            {simulatorCase.learningObjectives.slice(0, 3).map((objective, index) => (
                                                <li key={index} className="flex items-start">
                                                    <span
                                                        className="inline-block w-1.5 h-1.5 bg-blue-500 rounded-full mt-2 mr-2 flex-shrink-0"></span>
                                                    <span>{objective}</span>
                                                </li>
                                            ))}
