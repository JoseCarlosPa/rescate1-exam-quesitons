                            ...
                            {lections.map(page => {
                                ...
                                return (
                                    <button key={page.key} onClick={() => setSelectedPage(page.key)}
                                        className={`w-full text-left px-4 py-2.5 flex items-center justify-between gap-2 transition-colors ${isActive ? 'bg-orange-50' : 'hover:bg-slate-50'
                                            }`}>
                                        <span className={`text-sm truncate flex-1 ${isActive ? 'font-bold text-orange-600' : 'text-slate-600 font-medium'}`}>
                                            {page.title}
                                        </span>
                                        <span className={`flex-shrink-0 text-xs font-bold px-1.5 py-0.5 rounded-full ${isActive ? 'bg-orange-200 text-orange-800' : 'bg-slate-100 text-slate-500'
                                            }`}>
                                            {statsLoading ? '·' : count}
                                        </span>
                                    </button>
