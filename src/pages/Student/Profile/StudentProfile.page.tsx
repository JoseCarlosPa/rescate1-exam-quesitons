...
import {StoredCert, certsSummary, certStatus, certStatusMeta, normalizeCerts} from '../../../utils/certifications';
...
interface StudentData {
    ...
    photoURL?: string;
    createdAt: Timestamp;
    // Datos operativos — solo presentes cuando role === 'Elemento'
    phone?: string;
    bio?: string;
    rank?: string;
...
export default function StudentProfile() {
    ...
    const handleSaveProfile = async () => {
        ...
        setSaving(true);
        try {
            const isElemento = studentData?.role === 'Elemento';
            // Los campos phone/bio solo tienen sentido (y permiso en firestore.rules)
            // para el personal operativo; para el resto no se envían.
            ...
            await updateDoc(doc(db, 'users', user.id), profileUpdate);
            if (auth.currentUser) {
            ...
            setStudentData(prev => prev ? {
                ...
                ...(isElemento ? {phone: phone.trim(), bio: bio.trim()} : {}),
            } : null);
            setNewPassword('');
            setConfirmPassword('');
    ...
    const isElemento = studentData?.role === 'Elemento';
    const certs = normalizeCerts(studentData?.certifications);
            ...
            <div className="px-4 md:px-8 lg:px-12 -mt-16 pb-12">
                <div className="max-w-2xl mx-auto space-y-5">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
                        ...
                        {/* Teléfono + Bio — solo personal operativo (Elemento) */}
                        {isElemento && (
                            <>
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 mt-1 ml-1">Visible para tu guardia para contacto rápido</p>
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Acerca de mí</label>
                                    <div className="relative">
                                        <FiFileText className="absolute left-3.5 top-3.5 text-slate-400 w-4 h-4"/>
                                        <textarea
                                            value={bio} onChange={e => setBio(e.target.value)} rows={3}
                                            placeholder="Especialidades, experiencia, notas..."
                                            className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-slate-200 text-slate-800 text-sm font-medium outline-none transition-all focus:border-orange-400 resize-none"
                                        />
                                    </div>
                                </div>
                            </>
                    ...
                    {/* Información operativa — solo Elemento (solo lectura, administrada por Jefe/Admin) */}
                    {isElemento && (
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
                            <h3 className="text-sm font-black text-slate-700 uppercase tracking-wide flex items-center gap-2">
                                <FiShield className="w-4 h-4 text-orange-500"/> Información operativa
                            </h3>

                            <div className="grid grid-cols-2 gap-3">
                                <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide flex items-center gap-1">
                                        <FiUsers className="w-3 h-3"/> Guardia
                                    </p>
                                    <p className="text-sm font-bold text-slate-800 mt-0.5">{studentData?.guardia || 'Sin asignar'}</p>
                                </div>
                                <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide flex items-center gap-1">
                                        <FiShield className="w-3 h-3"/> Papel
                                    </p>
                                    <p className="text-sm font-bold text-slate-800 mt-0.5">{studentData?.guardRole || '—'}</p>
                                </div>
                                <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide flex items-center gap-1">
                                        <FiAward className="w-3 h-3"/> Rango
                                    </p>
                                    <p className="text-sm font-bold text-slate-800 mt-0.5">
                                        {studentData?.rank === 'Avanzado' ? '⭐ Avanzado' : '🔰 Básico'}
                                    </p>
                                </div>
                                <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide flex items-center gap-1">
                                    ...
                                    <p className="text-sm font-bold text-slate-800 mt-0.5">{studentData?.graduationYear || '—'}</p>
                                </div>
                            </div>

                            {/* Alertas de certificaciones */}
                            {(certSummary.vencidas > 0 || certSummary.porVencer > 0) && (
                                <div className="flex items-center gap-2 p-3 rounded-xl"
                                     style={{background: certSummary.vencidas > 0 ? '#fee2e2' : '#fef3c7'}}>
                                    <FiAlertTriangle className="w-4 h-4 flex-shrink-0"
                                                     style={{color: certSummary.vencidas > 0 ? '#dc2626' : '#d97706'}}/>
                                    <p className="text-xs font-semibold"
                                       style={{color: certSummary.vencidas > 0 ? '#991b1b' : '#92400e'}}>
                                        {certSummary.vencidas > 0 && `${certSummary.vencidas} certificación(es) vencida(s). `}
                                        {certSummary.porVencer > 0 && `${certSummary.porVencer} por vencer pronto.`}
                                    </p>
                                </div>
                            )}
                            ...
                            <div>
                                ...
                                {certs.length === 0 ? (
                                    <p className="text-sm text-slate-400">Aún no tienes certificaciones registradas. Pídele a tu Jefe de guardia que las agregue.</p>
                                ) : (
                            ...
                            <p className="text-xs text-slate-400 flex items-center gap-1">
                                ...
                                Estos datos los administra tu Jefe de guardia o un Administrador.
                            </p>
                        </div>
                    )}
