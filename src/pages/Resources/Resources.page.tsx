...
export default function Resources() {
    ...
    return (
        ...
        >
            <div
                className="flex flex-col min-h-screen bg-gradient-to-br from-orange-50 to-gray-200 pb-12 md:p-12 px-4 p-4">
                <div className="flex flex-col justify-center items-center mb-8">
                    <p className="text-5xl font-extrabold mb-2 text-orange-600 drop-shadow text-center">Recursos y
                        Libros</p>
                ...
                <div
                    className="grid 2xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-2 grid-cols-1 gap-10 mt-6">
                    {filtered.length === 0 && (
                        <div className="col-span-full text-center text-gray-500 text-xl">No se encontraron
                            recursos.</div>
                    )}
                    {filtered.map((r, i) => (
                        <div key={i}
                             className="bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-shadow duration-300 flex flex-col items-center p-6 group border border-orange-100">
                            <img src={r.img}
                                 className="mb-4 w-40 h-52 object-cover rounded-xl group-hover:scale-105 transition-transform duration-300"
                                 alt={r.title}/>
                            <div className="flex flex-col gap-2 w-full items-center">
                                ...
                                {r.edition &&
                                    <p className="text-sm text-gray-700"><strong>Edición:</strong> {r.edition}</p>}
                                {r.editor &&
                                    <p className="text-sm text-gray-700"><strong>Editor:</strong> {r.editor}</p>}
                                {r.category && <span
                                    className="text-xs bg-orange-100 text-orange-700 px-3 py-1 rounded-full mt-1">{r.category}</span>}
